...
class TSEncoder(nn.Module):
    ...
    # forward_time 改为非原地操作
    def forward_time(self, x, mask=None):  # x: B x T x input_dims
        x = x.transpose(1, 2)
        nan_mask = ~x.isnan().any(axis=-1)
        x = torch.where(nan_mask.unsqueeze(-1), x, torch.zeros_like(x))  # 非原地操作
        x = self.input_fc(x)  # B x T x Ch
        ...
        mask = mask & nan_mask  # 非原地操作
        x = torch.where(mask.unsqueeze(-1), x, torch.zeros_like(x))  # 非原地操作
